...
- [~] **Phase 5** — Add Ableton and FL Studio workflow entries at scale. In progress: 61 entries (up from 14) — 28 Ableton, 24 FL Studio, and a cross-DAW `daw/workflow/` folder (9 entries) covering session/live-performance, automation/modulation, templates, recording/comping, mixing, mastering, sampling, mix troubleshooting, vocal tuning, orchestral MIDI programming, sound-design automation, and genre-specific arrangement (EDM/trap/house-techno/dubstep) workflows.
