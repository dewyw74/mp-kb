...
Process:
1. Search `knowledge_base/midi/{patterns,controllers,programming}` (Grep/Glob) for relevant pattern and controller-mapping guidance, and check `knowledge_base/genres/` for genre-specific MIDI programming notes (bass patterns, drum patterns, velocity/humanization conventions). If nothing exists yet, say so and reason from general MIDI programming practice.
