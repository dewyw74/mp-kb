...
Process:
1. Search `knowledge_base/sound_design/{synthesis,sampling,effects,presets}` (Grep/Glob) for relevant patch-design guidance and any genre-specific sound design notes in `knowledge_base/genres/`. If nothing exists yet, say so and design from synthesis fundamentals.
