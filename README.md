...
- [~] **Phase 5** — Add Ableton and FL Studio workflow entries at scale. In progress: 91 entries (up from 14) — 40 Ableton, 36 FL Studio, and a cross-DAW `daw/workflow/` folder (15 entries) covering session/live-performance, automation/modulation, templates, recording/comping, mixing, mastering, sampling, mix troubleshooting, vocal chain (tuning through full signal chain), drum-bus compression, orchestral MIDI programming, sound-design automation, granular/resampling, sidechain routing, stem-export/collaboration handoff, DI/reamping, MIDI chord/arpeggiator tools, vinyl mastering export prep, and genre-specific arrangement (EDM/trap/house-techno/dubstep/lofi/DnB/trance/ambient) workflows.
